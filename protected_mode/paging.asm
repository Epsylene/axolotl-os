...
; entry points to a page table ; in the page table, each
; entry points to a 4 KB physical page frame (that is, a 4
; KB segment of physical memory). Translating a virtual
...
; tables are used : the page-map level-4 table (PML4T),
; which is above the page-directory pointer table (PDPT),
; which is above the page-directory table (PDT), which is
; above the page table (PT). Entries are now 64 bits long,
; so there are only 512 of each, and the total maximum
; physical memory adressable is 512 GB per PML4T entry, so
; 256 TB.
...
set_up_identity_paging:
    pushad ; PUSHAD, like PUSHA, pushes all registers on
        ; the stack, except it does so with the 32-bit
        ; variants instead of the 16-bit ones.

    ; The first thing we will do is an "identity paging"
    ; (that is, a 1:1 paging, where virtual and physical
    ; adresses are the same) of the first megabyte of
    ; memory, to make things easier next. We start by
    ; clearing the memory area of the page tables:
    ...
    mov dword [edi], 0x2003 ; set the adress 0x1000 to the
        ...
        ;   feature allows to set different memory
        ;   attributes for the page.
        ;
        ; Here, because the first two bits are set (3 is
        ; 011), the page is present and read/write-able.
    add edi, 0x1000
    mov dword [edi], 0x3003 ; PDPT -> PDT
    add edi, 0x1000
    mov dword [edi], 0x4003 ; PDT -> PT

    ; Next we add 512 entries to our page table, such that
    ; each contains the corresponding physical adress (PT[0]
    ; = 0x0, PT[1] = 0x1, etc):
    add edi, 0x1000
    ...
    ; processors support less than this amount) and is
